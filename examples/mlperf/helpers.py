...
def get_fake_data_bert(BS:int):
  return {
    "input_ids": Tensor.empty((BS, 512), dtype=dtypes.float32).contiguous(),
    "input_mask": Tensor.empty((BS, 512), dtype=dtypes.default_float).contiguous(),
    "segment_ids": Tensor.empty((BS, 512), dtype=dtypes.float32).contiguous(),
    "masked_lm_positions": Tensor.empty((BS, 76), dtype=dtypes.float32).contiguous(),
    "masked_lm_ids": Tensor.empty((BS, 76), dtype=dtypes.float32).contiguous(),
    "masked_lm_weights": Tensor.empty((BS, 76), dtype=dtypes.float32).contiguous(),
    "next_sentence_labels": Tensor.empty((BS, 1), dtype=dtypes.float32).contiguous(),
  }
