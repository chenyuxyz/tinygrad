...
def get_rangeify_map(sink:UOp, raw_dep_buffers:set[UOp]|None=None) -> dict[UOp, UOp]:
  ...
  if _raw_dep:
    ...
    kernel_writers: dict[UOp, UOp] = {}
    for u in tsink.toposort():
      if u.op is Ops.AFTER: kernel_writers[u.buf_uop] = u
    ...
    for u in tsink.toposort():
      ...
      for s in kernel_srcs:
        if s.op is not Ops.BUFFER or s is u.buf_uop or s not in _raw_dep or (a:=kernel_writers.get(s)) is None: continue
        if u in a.toposort(): continue  # skip: writer already depends on this kernel, adding RAW dep would create cycle
        extra_deps.append(a)
      if extra_deps:
  ...
  for u in tsink.toposort():
    ...
    for s in u.src[1].src:
      ...
      if s.op is not Ops.BUFFER or s is u.buf_uop or s in _raw_dep or (a:=kernel_assign2.get(s)) is None: continue
