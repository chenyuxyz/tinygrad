...
def assign_to_contiguous(assign:UOp, target:UOp, src:UOp):
  ...
  if target is not t and target.op_in_backward_slice_with_self(Ops.SHRINK):
    # base already realized: copy src to prevent overlapping read/write hazard (fix_assign_hazard can't see through CONTIGUOUS gate)
    if t.op is Ops.CONTIGUOUS: return assign.replace(src=(target, src.contiguous()))
