...
pm_gradient = PatternMatcher([
  ...
  (UPat(Ops.ALLREDUCE, name="ret"), lambda ctx, ret: (ctx.copy_to_device(ret.src[0].device), None)),
  ...
  (UPat(Ops.MSTACK, name="ret"), lambda ctx, ret: tuple(ctx.mselect(i) for i in range(len(ret.src)))),
