...
def packed_store(bidx:UOp, var:UOp):
  ...
  return UOp.store(UOp(Ops.INDEX, bidx.dtype, idx_src), ((buf & wmask) | new_v.cast(dtypes.uint32)))
...
class WGSLRenderer(CStyleLanguage):
  ...
  string_rewrite = PatternMatcher([
    ...
    (UPat(Ops.BITCAST, dtype=(dtypes.char, dtypes.uchar), name="x"),
     lambda ctx,x: f"bitcast<{ctx.type_map[x.dtype]}>({ctx[x.src[0]]}&{_mask(x.dtype)})"),
    (UPat(Ops.BITCAST, dtype=(dtypes.short, dtypes.ushort), name="x"), lambda ctx,x: f"bitcast<{ctx.type_map[x.dtype]}>(vec2<f16>({ctx[x.src[0]]},0))"
     if x.src[0].dtype == dtypes.half else f"bitcast<{ctx.type_map[x.dtype]}>({ctx[x.src[0]]}&{_mask(x.dtype)})"),
