...
def get_out_buffers_for_ei(ei:ExecItem) -> list[Buffer]:
  if isinstance(ei.prg, CompiledRunner):
    ret = [ei.bufs[out] for out in ei.prg.p.outs if out not in ei.prg.p.ins]
    assert all(b is not None for b in ret)
    return ret  # type: ignore[return-value]
  if isinstance(ei.prg, (BufferCopy, BufferXfer, EncDec)):
    assert ei.bufs[0] is not None
    return [ei.bufs[0]]
  return []
