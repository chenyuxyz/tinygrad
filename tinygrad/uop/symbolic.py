...
symbolic_simple = propagate_invalid + PatternMatcher([
  ...
  (UPat.var("x", dtype=dtypes.bool).where(UPat.cvar("t", arg=True), UPat.cvar("f", arg=False)), lambda x,t,f: x.cast(t.dtype)),
  (UPat.var("x", dtype=dtypes.bool).where(UPat.cvar("f", arg=False), UPat.cvar("t", arg=True)), lambda x,t,f: x.logical_not().cast(t.dtype)),
