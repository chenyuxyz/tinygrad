...
def simplify_valid(valid:UOp) -> UOp|None:
  ...
  for stmt in sorted(valids, key=lambda v: _valid_priority(v, valids)):
    # NOTE: uop_given_valid only parses CMPNE and CMPLT, skipping conditions is still correct in ANDs
    if ret and stmt.op in (Ops.CMPNE, Ops.CMPLT) and (newstmt:=uop_given_valid(functools.reduce(operator.and_, ret), stmt)) is not None:
      ret.append(newstmt)
      if newstmt is not stmt: something_changed = True
    else: ret.append(stmt)
