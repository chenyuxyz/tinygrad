  ...
  def _min_max(self) -> tuple[ConstType, ConstType]:
    if self.op in GroupOp.Binary and not dtypes.is_float(self.dtype):
      ...
      if self.op is Ops.AND and dtypes.is_int(self.dtype):
        if s1_vmin == s1_vmax >= 0: return 0, s1_vmax if s0_vmin < 0 else min(s0_vmax, s1_vmax)
        if s0_vmin == s0_vmax >= 0: return 0, s0_vmax if s1_vmin < 0 else min(s0_vmax, s1_vmax)
