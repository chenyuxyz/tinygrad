...
class UOp(OpMixin, metaclass=UOpMetaClass):
  ...
  def _min_max(self) -> tuple[PyConst, PyConst]:
    ...
    if self.op is Ops.WHERE:
      if self.src[2].op is Ops.CONST and self.src[2].arg is Invalid: return self.src[1]._min_max
      if dtypes.is_int(self.dtype): return min(self.src[1].vmin, self.src[2].vmin), max(self.src[1].vmax, self.src[2].vmax)
