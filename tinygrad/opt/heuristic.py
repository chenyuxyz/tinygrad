...
def hand_coded_optimizations(k:Kernel) -> list[Opt]:
  ...
  if k.first_reduce < k.first_upcast and \
    ...
    if isinstance(s:=k.full_unupcasted_shape[-1], int) and 1 < s <= 32:  # NOTE: cannot loop unroll symbolic axis
      k.apply_opt(Opt(OptOps.UNROLL, len(k.full_unupcasted_shape)-1-k.first_reduce, 0))
      # if it's small, upcast a second reduce dimension too
      if k.first_reduce < k.first_upcast and 1 < s <= 3 and isinstance(s2:=k.full_unupcasted_shape[-1], int) and 1 < s2 <= 3:
        k.apply_opt(Opt(OptOps.UNROLL, len(k.full_unupcasted_shape)-1-k.first_reduce, 0))
    else:
      for splits in [4]:
