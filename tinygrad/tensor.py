...
class Tensor(MathTrait):
  ...
  def logcumsumexp(self, axis=0) -> Tensor:
    ...
    x_cummax = x.cummax(-1)
    x_unsqueezed = x.unsqueeze(-2).expand((None,)*(self.ndim-1)+(x_cummax.shape[-1], None))
    # subtract cummax along axis before exp for numerical stability
    ret = ((x_unsqueezed - x_cummax.unsqueeze(-1)).exp()).tril().sum(-1).log() + x_cummax
