...
class Tensor(OpMixin):
  ...
  def assign(self, x:Tensor|PyConst|list|tuple) -> Tensor:
    ...
    if is_disk:
      ...
      return self
    # chained full-buffer assign should continue writing into the original target buffer
    if self.uop.op is Ops.ASSIGN and (target:=self.uop.src[0]).has_buffer_identity():
      if self.uop in x.uop.toposort():
        # break assign-in-source cycles through a lazy temporary, then assign back to the original target
        result = self._apply_uop(lambda _self, val: target.assign(val.contiguous()), x)
      else:
        result = self._apply_uop(lambda _self, val: target.assign(val), x)
    else:
